...
DAMOS 配额目标用于反馈驱动的配额自动调优。DAMOS 会自动调整相应方案的配额，使所选
目标指标（``target_metric``）的当前值（``current_value``）趋近设定的目标值
（``target_value``）。配置配额目标时，如果 ``target_metric`` 为
``node_mem_used_bp``、``node_mem_free_bp``、``node_memcg_used_bp``、
``node_memcg_free_bp`` 或 ``node_eligible_mem_bp``，可以通过可选参数 ``nid`` 指定
NUMA 节点。如果 ``target_metric`` 为 ``node_memcg_used_bp`` 或
``node_memcg_free_bp``，还可以通过可选参数 ``path`` 指定 cgroup。该参数的值应为
内存 cgroup 相对于 cgroup 挂载点的路径。
